...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Print the exact LEARNING_PATH_MCP_SERVER_URL value for the current server mode."
    )
    parser.add_argument(
        "--transport",
    ...
    parser.add_argument("--port", type=int, default=8002, help="Server port (ignored when --public-base-url is set).")
    parser.add_argument(
        "--public-base-url",
        default="",
    ...
    if args.transport == "stdio":
        print("LEARNING_PATH_MCP_SERVER_URL is not applicable for stdio mode.")
    ...
    print(url)
    print(f"LEARNING_PATH_MCP_SERVER_URL={url}")
